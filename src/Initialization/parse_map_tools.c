...
/*   Updated: 2024/09/16 13:23:02 by arafa            ###   ########.fr       */
...
int	check_trigger(t_data *data, char *tab, int trigger)
{
	...
	if (!ft_strncmp("C", tab, 1) && trigger > 5)
		exit_error_message(data, "Error\nTextureProblem\n");
	if (ft_strncmp("C", tab, 1) && ft_strncmp("NO", tab, 2)
		&& ft_strncmp("SO", tab, 2) && ft_strncmp("WE", tab, 2)
		&& ft_strncmp("EA", tab, 2) && ft_strncmp("F", tab, 1)
		&& ft_strncmp("\n", tab, 1))
		exit_error_message(data, "Error\nTextureProblem\n");
	return (0);
}
